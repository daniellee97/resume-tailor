...
export default function EditorPage({ resumeData: initialData, resumeText, analysis, onReset }: Props) {
  const [activeTab, setActiveTab] = useState(0);
  const [data, setData] = useState<ResumeData>(initialData);
  const [latexBody, setLatexBody] = useState(() => generateLatexBody(initialData));
  ...
  // ── Vertical split inside left panel (editor % of left panel height) ───
  ...
  // Regenerate LaTeX whenever form data changes
  useEffect(() => {
  ...
  const hDivider = {
    ...
    bgcolor: '#e5e7eb', transition: 'background 0.15s',
      ...
      <Box ref={containerRef} sx={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' }}>
        ...
        >
          ...
          <Box sx={{ height: `${editorPct}%`, minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            {activeTab === 0 && <ResumeEditorPanel data={data} onChange={setData} />}
            {activeTab === 1 && (
          ...
          <Box sx={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
            <ChatPanel resumeText={resumeText} initialAnalysis={analysis} />
          </Box>
        </Box>
