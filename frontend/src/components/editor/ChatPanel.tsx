...
import {
  Box, Typography, TextField, IconButton, CircularProgress, Avatar,
} from '@mui/material';
import {
  Send as SendIcon,
...
export default function ChatPanel({ resumeText, initialAnalysis }: Props) {
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await sendChatMessage(text, history, resumeText);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: '#ffffff' }}>
      ...
      {/* Header */}
      <Box sx={{
        px: 2, py: 1, borderBottom: '1px solid #e5e7eb',
        ...
        {messages.map((msg, i) => (
          ...
          >
            ...
            <Avatar sx={{
              ...
              bgcolor: msg.role === 'assistant' ? '#4f46e5' : '#e5e7eb',
            }}>
              {msg.role === 'assistant'
      ...
      {/* Input */}
      <Box sx={{
        p: 1.5, borderTop: '1px solid #e5e7eb',
        ...
        <TextField
          ...
          sx={{
            '& .MuiOutlinedInput-root': {
              fontSize: '0.83rem', borderRadius: 2, bgcolor: '#f9fafb',
            },
          }}
        />
        <IconButton
